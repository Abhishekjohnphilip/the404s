...
import WishWall from './wish-wall';
import { Calendar, Cake } from 'lucide-react';
import AdminToolbarWrapper from './admin-toolbar-wrapper';
...
export default async function BirthdayPage({
  ...
  const person = await getEventBySlug(year, slug);

  if (!person || person.type !== 'birthday') {
    notFound();
  }
    ...
    <div className="bg-secondary">
      <div className="container mx-auto px-4 sm:px-6 lg:px-8 py-12">
        ...
        <div className="relative text-center rounded-lg p-8 bg-card shadow-lg mb-12">
          <AdminToolbarWrapper person={person} year={year} isAdmin={isAdmin} />
          ...
          <div className="mt-4 flex items-center justify-center gap-4 text-muted-foreground text-lg">
            ...
            <div className="flex items-center gap-2">
              ...
              <span>Celebrating another great year!</span>
            </div>
          </div>
        </div>

        {person.media.length > 0 && <MediaGallery media={person.media} />}

        <WishWall person={person} isAdmin={isAdmin} year={year} />
      </div>
    </div>
  );
}
